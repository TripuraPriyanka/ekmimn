...
def preprocess_SNLI_data(inputdir,
    ...
    print("\twordpair making")
    feat_path = os.path.join(targetdir, "pair_features.txt")
    bk_for_x = preprocessor.features2pkl(feat_path, )
    with open(os.path.join(targetdir, "pair_features.pkl"), "wb") as pkl_file:
        pickle.dump(bk_for_x, pkl_file)
