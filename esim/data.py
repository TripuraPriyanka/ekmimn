...
class Preprocessor(object):
    ...
    def features2pkl(self, feat_path):
        bk_for_x = {}
        np.random.seed(1234)
        with open(feat_path, 'r') as f1:
            for line in f1:
                l = line.strip().split(' ')
                ids = l[0].split(';')
                if ids[0] in self.worddict and ids[1] in self.worddict:
                    ids0 = self.worddict[ids[0]]
                    ids1 = self.worddict[ids[1]]
        ...
        print('feature2pkl size, {}, {}'.format(len(bk_for_x.keys()), count))
        return bk_for_x
